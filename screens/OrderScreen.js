...
import { View, Text, StyleSheet, TextInput, Image, TouchableOpacity, ToastAndroid, ActivityIndicator } from "react-native";
    ...
    confirmOrder = async () => {
        if (this.state.internetConnected) {
            try {
                ...
                let response = await fetch(`http://192.168.0.152:8080/restaurant/confirm-order?userId=${encodeURIComponent(data.userId)}&orderId=${encodeURIComponent(data.orderId)}`, {
                    method: 'POST',
                    headers: new Headers({
                        'Content-Type': 'application/json',
                        'Authorization': 'Bearer ' + token,
                ...
                {this.state.loading === true
                    ? <View>
                        <View style={{ marginTop: 100 }}>
                            <Text style={styles.headingText}>ZAMÓWIENIE</Text>
                            <Text style={styles.subtitleText}>Trwa tworzenie zamówienia...</Text>
                        </View>
